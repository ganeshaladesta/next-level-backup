...
const Store = (() => {
  ...
  /* ---------- Services ---------- */
  function getServices() { return _get(SERVICES_KEY); }
  ...
  function addService({ name, price }) {
    ...
    const svc = { id: _id('svc'), name, price: Number(price), active: true };
    list.push(svc);
  ...
  function updateService(id, updates) {
    ...
    if (updates.price !== undefined) list[idx].price = Number(updates.price);
    _set(SERVICES_KEY, list);
    return list[idx];
  }

  function deleteService(id) {
    _set(SERVICES_KEY, getServices().filter(s => s.id !== id));
  }
  ...
  function addTransaction({ branch, serviceId, serviceName, price, date, notes, promoId, promoDiscount }) {
  ...
  function deleteTransaction(id) {
    _set(TRANSACTIONS_KEY, _get(TRANSACTIONS_KEY).filter(t => t.id !== id));
  }
  ...
  /* ---------- Promos ---------- */
  function getPromos() { return _get(PROMOS_KEY); }
  ...
  function updatePromo(id, updates) {
    ...
    if (idx === -1) return null;
    list[idx] = { ...list[idx], ...updates };
    if (updates.discount !== undefined) list[idx].discount = Number(updates.discount);
    _set(PROMOS_KEY, list);
    return list[idx];
  }

  function deletePromo(id) {
    _set(PROMOS_KEY, getPromos().filter(p => p.id !== id));
  }
  ...
  /* ---------- public API ---------- */
  return {
    getServices, getActiveServices, getServiceById,
    addService, updateService, deleteService,
    getTransactions, addTransaction, updateTransaction, deleteTransaction,
    getTransactionsByDateRange,
    getPromos, getActivePromos, addPromo, updatePromo, deletePromo,
    init, formatCurrency, getTodayStr, formatDate
  };
})();
