...
const Transactions = (() => {
  ...
  /* ==================== Transaction Form ==================== */
  function render() {
    if (!_initTxn) { _setupTxnForm(); _initTxn = true; }
  ...
  function _updatePromoOptions() {
    ...
    sel.innerHTML = '<option value="">Tanpa Promo</option>' +
      promos.map(p =>
  ...
  function _saveTransaction() {
    ...
    if (!svcSel.value) { showToast('Pilih layanan terlebih dahulu!', 'warning'); return; }
    if (!priceIn.value || Number(priceIn.value) <= 0) { showToast('Masukkan harga yang valid!', 'warning'); return; }
    ...
    if (editingId) {
      ...
      editingId = null;
      document.getElementById('txnSubmitBtn').textContent = '💾 Simpan Transaksi';
  ...
  function _renderRecent() {
    ...
    if (txns.length === 0) {
      el.innerHTML = '<p class="empty-state">Belum ada transaksi. Mulai tambahkan transaksi pertama! 💰</p>';
      return;
    }
  ...
  function edit(id) {
    ...
    const origPrice = txn.promoDiscount > 0
  ...
  function renderHistory() {
    if (!_initHist) { _setupHistory(); _initHist = true; }
    _renderHistoryTable();
  }
  ...
  function exportCSV() {
    ...
    if (txns.length === 0) { showToast('Tidak ada data untuk di-export!', 'warning'); return; }
