...
const Dashboard = (() => {
    ...
    if (active.length > 0) {
      banner.innerHTML = active
        .map(
          ...
          <span>
            <strong>${p.name}</strong> — ${p.discount}% Off${p.description ? " · " + p.description : ""}
          </span>
        </div>`,
        )
        .join("");
  ...
  function renderSummaryCards() {
    ...
    _setText(
      "totalServices",
      Store.getActiveServices().length + " Active Services",
    );
  }
  ...
  function _getServiceData() {
    ...
    txns.forEach((t) => {
      smap[t.serviceName] = (smap[t.serviceName] || 0) + t.price;
    });
