...
const Dashboard = (() => {
  ...
  function _setup() {
    ...
    if (branchFilter) {
      BRANCHES.forEach((branch) => {
        if (
          ![...branchFilter.options].some((option) => option.value === branch)
        ) {
  ...
  async function renderSummaryCards() {
    ...
    if (isRevenue) {
      ...
      _setText("todayCount", `Revenue: ${Store.formatCurrency(todayRevenue)}`);

      _setText("monthRevenue", `${monthTxns.length} Transactions`);

      _setText("monthCount", `Revenue: ${Store.formatCurrency(monthRevenue)}`);

      _setText("yearRevenue", `${yearTxns.length} Transactions`);

      _setText("yearCount", `Revenue: ${Store.formatCurrency(yearRevenue)}`);
    }

    _setText("totalTransactions", filtered.length);
    ...
    _setText("dashTotalRevenue", Store.formatCurrency(revenue));

    _setText("dashTotalTransactions", count.toLocaleString("id-ID"));

    _setText("dashAverageTicket", Store.formatCurrency(averageTicket));

    _setText("dashTotalDiscount", Store.formatCurrency(discount));
    ...
    _setText("dashPromoRevenue", Store.formatCurrency(promoRevenue));

    _setText("dashPromoDiscount", Store.formatCurrency(promoDiscount));
    ...
    _setText("dashPeriodLabel", periodLabel);

    _setText("todayCardLabel", currentFilter === "daily" ? "Today" : "Today");

    _setText("monthCardLabel", "This Month");

    _setText("yearCardLabel", "This Year");
  }
  ...
  async function _getServiceData() {
    ...
    txns.forEach((t) => {
      const name = t.serviceName || "Unknown Service";

      if (!map[name]) {
        map[name] = 0;
      }

      map[name] += currentMetric === "revenue" ? Number(t.price || 0) : 1;
    });
    ...
    const colors = labels.map(
      (_, i) => _chartColors()[i % _chartColors().length],
    );
  ...
  async function renderPromoPerformance() {
    ...
    const promoTxns = txns.filter(isPromoTransaction);

    const normalTxns = txns.filter((t) => !isPromoTransaction(t));

    const promoRevenue = sumRevenue(promoTxns);

    const normalRevenue = sumRevenue(normalTxns);

    const promoDiscount = promoTxns.reduce(
      (sum, t) => sum + Number(t.discountAmount || 0),
      0,
    );

    const normalAverage = normalTxns.length
      ? normalRevenue / normalTxns.length
      : 0;

    const promoAverage = promoTxns.length ? promoRevenue / promoTxns.length : 0;

    /* ----------------------------------------------------------
         Existing / optional KPI elements
         ---------------------------------------------------------- */

    _setText("promoRevenue", Store.formatCurrency(promoRevenue));

    _setText("nonPromoRevenue", Store.formatCurrency(normalRevenue));

    _setText("promoTransactionCount", promoTxns.length);

    _setText("nonPromoTransactionCount", normalTxns.length);

    _setText("promoAverageTicket", Store.formatCurrency(promoAverage));

    _setText("nonPromoAverageTicket", Store.formatCurrency(normalAverage));

    _setText("promoDiscountTotal", Store.formatCurrency(promoDiscount));

    /* ----------------------------------------------------------
         Promo Performance Table
         ---------------------------------------------------------- */

    const container = document.getElementById("promoPerformance");
    ...
    promoTxns.forEach((t) => {
      ...
      promoMap[promoId].transactions++;

      promoMap[promoId].revenue += Number(t.price || 0);

      promoMap[promoId].discount += Number(t.discountAmount || 0);
    });

    const rows = promos
      .filter((p) => promoMap[p.id])
      .map((p) => {
        const item = promoMap[p.id];
        ...
        return `
            <tr>
              ...
              <td data-label="Revenue">
                ${Store.formatCurrency(item.revenue)}
              </td>
  
              <td data-label="Discount">
                ${Store.formatCurrency(item.discount)}
              </td>
  
              <td data-label="Avg Ticket">
                ${Store.formatCurrency(item.revenue / item.transactions)}
              </td>
            </tr>
          `;
      })
  ...
  function filterBranch(data) {
    ...
    return data.filter(
      (t) =>
        String(t.branch || "").toLowerCase() ===
        String(currentBranch).toLowerCase(),
    );
  }
  ...
  function getPeriodTransactions(data) {
    ...
    if (currentFilter === "daily") {
      return data.filter((t) => t.date === today);
    }
    ...
    if (currentFilter === "monthly") {
      ...
      return data.filter((t) => t.date >= start && t.date <= today);
    }
  ...
  function sumRevenue(transactions) {
    return transactions.reduce((total, t) => total + Number(t.price || 0), 0);
  }
